...
interface Product {
  ...
  lastUpdated: string;
  minStock: number;
  losses: number; // mermas acumuladas (unidades)
}
...
export default function InventoryPage() {
  ...
  const [variantFilterSize, setVariantFilterSize] =
    useState<string>("all");
  const [variantFilterColor, setVariantFilterColor] =
    useState<string>("all");
  const [variantFilterStatus, setVariantFilterStatus] =
    useState<string>("all");
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesStatus =
      productFilterStatus === "all" || status === productFilterStatus;
  ...
  const filteredVariants = variants.filter((variant) => {
    ...
    const matchesSize =
      variantFilterSize === "all" || variant.size === variantFilterSize;
    const matchesColor =
      variantFilterColor === "all" || variant.color === variantFilterColor;

    const status = getVariantStockStatus(variant.stock).label;
    const matchesStatus =
      variantFilterStatus === "all" || status === variantFilterStatus;
  ...
  const productStats = {
    ...
    critical: products.filter(
      (p) => getStockStatus(p.stock, p.minStock) === "critical"
    ).length,
    out: products.filter(
      (p) => getStockStatus(p.stock, p.minStock) === "out"
    ).length,
    lossesUnits: products.reduce((sum, p) => sum + (p.losses || 0), 0),
  };

  // Stats para variantes
  const variantStats = {
    available: variants.filter(
      (v) => getVariantStockStatus(v.stock).label === "Disponible"
    ).length,
    low: variants.filter(
      (v) => getVariantStockStatus(v.stock).label === "Bajo"
    ).length,
    out: variants.filter(
      (v) => getVariantStockStatus(v.stock).label === "Agotado"
    ).length,
  };
    ...
    <AdminDashboardLayout user={user}>
      <div className="space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="bg-card border border-border rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-red-400">
                      {productStats.lossesUnits}
                    </p>
                  </div>
                  <TrendingDown className="h-8 w-8 text-red-400 rotate-180" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-lg p-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Select
                  ...
                  onValueChange={setProductFilterStatus}
                >
                          ...
                          <TableCell>
                            <div className="flex items-center justify-end gap-2">
                              <Button
                                ...
                                onClick={() =>
                                  router.push(
                                    `/dashboard/inventory/${product.id}/edit`
                                  )
                                }
                              >
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  router.push(
                                    `/dashboard/inventory/${product.id}/history`
                                  )
                                }
                              >
          ...
          {/* Tab: Vista por Variantes (sin cambios de mermas) */}
